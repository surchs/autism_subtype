...
def hnu1_weight_stability_fixed(scale, n_cpu):
    # Hardcoded variables
    n_subtypes = 3
    ...
    # Data
    resid_p = root_p / 'processed/residuals/hnu_1' / f'hnu1_session_{{}}_mist_{scale}_residuals_after_{"_and_".join(regressors.split("+"))}.npy'
    labels_p = mask_root_p / f'Parcel_Information/MIST_{scale}.csv'
    # Output
    out_d = root_p / 'processed/stability/hnu_1/'
    array_out_p = out_d / f'hnu_1_weight_stabilities_mist_{scale}_fixed_{n_subtypes}_subtypes.npy'
    table_out_p = out_d / f'hnu_1_weight_stabilities_mist_{scale}_fixed_{n_subtypes}_subtypes.tsv'
    if not out_d.is_dir():
        out_d.mkdir()
    ...
    table_list = [job_table.assign(**{'icc': res_array[idx, 0, :],
                                      ...
                                      'network':labels.iloc[idx]['label']}) for idx in range(scale)]
...
if __name__ == "__main__":
    ...
    hnu1_weight_stability_fixed(args.scale, args.number_cpu)