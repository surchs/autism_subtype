...
def abide2_scrub(n_cpu):
    # Paths
    project_root_p = pal.Path('/project/6003287/DATA/ABIDE_2')
    prep_d = project_root_p / 'PREPROCESS_NIAK/'
    out_d = project_root_p / 'PREPROCESS_NIAK/fmri_scrubbed/'
    if not out_d.is_dir():
        out_d.mkdir()
    ...
    with futures.ProcessPoolExecutor(max_workers=n_cpu) as executor:
