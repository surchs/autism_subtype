...
qc_table_p = file.path(root_p, 'ABIDE1_Pheno_QC_pass.tsv')
psm_table_p = file.path(root_p, 'ABIDE1_Pheno_PSM_matched.tsv')
...
covariates = c('site_collapse','SUB_ID', 'DX_NUM', 'AGE_AT_SCAN', 'fd_scrubbed')

sites <- unique(qc_data$site_collapse)
count = 0
for (site in sites) {
  slice_data = subset(qc_data, site_collapse==site, select=covariates)
